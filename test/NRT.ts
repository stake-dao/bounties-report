...
const sdTokens = ["0xD1b5651E55D4CeeD36251c61c50C889B36F6abB5"];
...
const processClaims = async (contract: any, tokenContract: any, signer: any, merkleData: any) => {
  ...
  for (const [userAddress, data] of Object.entries(merkleData.merkle)) {
    ...
    if (amount > 0n) {
      console.log(`Processing claim for ${userAddress}`)

      const balanceBefore = await tokenContract.read.balanceOf([userAddress])

      await contract.write.claim([sdTokens[0], BigInt(data.index), userAddress, amount, data.proof], {
      ...
      console.log(`Claimed ${amount} tokens for ${userAddress}`)
    }
  }
...
describe('Deployed Contract Tests', function () {
  it('should process claims for both old and new merkle roots', async function () {
    const { contract, tokenContract, signer } = await loadFixture(setupTest)

    const merkleOldData = JSON.parse(fs.readFileSync(path.join(__dirname, 'merkle_old.json'), 'utf-8'))
    const merkleNewData = JSON.parse(fs.readFileSync(path.join(__dirname, 'merkle.json'), 'utf-8'))

    const targetOldMerkle = merkleOldData.find(item => item.address === sdTokens[0])
    const targetNewMerkle = merkleNewData.find(item => item.address === sdTokens[0])
    ...
    const oldClaimedAmounts = await processClaims(contract, tokenContract, signer, targetOldMerkle)
    console.log('Old claimed amounts:', oldClaimedAmounts)
    ...
    const newClaimedAmounts = await processClaims(contract, tokenContract, signer, targetNewMerkle)

    // Compare old and new claimed amounts
    const allAddresses = new Set([...Object.keys(oldClaimedAmounts), ...Object.keys(newClaimedAmounts)])
