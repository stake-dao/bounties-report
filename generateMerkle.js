...
const main = async () => {
  ...
  for (const space of Object.keys(proposalIdPerSpace)) {
    ...
    // Handle Pendle => Two last proposals; and period specified along the protocol name
    else {
      allPeriods = Object.keys(csvResult["sdpendle.eth"]);
      const proposalsPeriods = await fetchProposalsIdsBasedOnPeriods(SDPENDLE_SPACE, allPeriods);

      const tokenPrice = await getTokenPrice("sdpendle.eth");

      const userRewards = {};

      // Compute period per period, based on proposals ids
      for (const period in proposalsPeriods) {
        const id = proposalsPeriods[period];

        const csvResultFiltered = csvResult["sdpendle.eth"][period];
        ...
        console.log('CSV Result', csvResultFiltered);

        console.log('All addresses per choice', allAddressesPerChoice);
        ...
        // TODO : To test, don't work for now because some gauges on report not present in proposals
        const addressesPerChoice = getChoiceWhereExistsBribe(allAddressesPerChoice, csvResultFiltered);
        ...
        if (delegationVote) {
          ...
          if (delegationVote.vp > 0) {
            let delegationAPR = ((Number(delegationVote.totalRewards) * 26 * tokenPrice) / delegationVote.vp) * 100 / tokenPrice;
            delegationAPRs["sdpendle.eth"] = delegationAPR;
          }
        }
      ...
      } // End of for period in proposalsPeriods

      // New distribution is split here
      // We have to sum with old distribution if users don't claim
      const tokenToDistribute = SPACES_TOKENS["sdpendle.eth"];

      const lastMerkle = lastMerkles.find((m) => m.address.toLowerCase() === tokenToDistribute.toLowerCase());
      const network = SPACE_TO_NETWORK["sdpendle.eth"];
      ...
      newMerkles.push({
        "symbol": SPACES_SYMBOL["sdpendle.eth"],
        "address": tokenToDistribute,
        "image": SPACES_IMAGE["sdpendle.eth"],
        "merkle": merkle,
        root: merkleTree.getHexRoot(),
        "total": totalAmount,
        "chainId": parseInt(SPACE_TO_CHAIN_ID["sdpendle.eth"]),
        "merkleContract": NETWORK_TO_MERKLE[network]
      });
...
const getChoiceWhereExistsBribe = (addressesPerChoice, csvResult) => {
  const newAddressesPerChoice = {};
  if (!csvResult) {
    return newAddressesPerChoice;
  }

  const csvResultLowerCase = {};
  for (const key of Object.keys(csvResult)) {
    csvResultLowerCase[key.toLowerCase()] = csvResult[key];
  }

  const addresses = Object.keys(csvResultLowerCase).map((addr) => addr.toLowerCase());
  ...
  for (const key of Object.keys(addressesPerChoice)) {
    ...
    for (const addr of addresses) {
      ...
      newAddressesPerChoice[addr] = {
        ...
        amount: csvResultLowerCase[addr]
      };
      break;
    }
  }
