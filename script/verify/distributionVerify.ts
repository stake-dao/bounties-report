...
The "vlCVX Deterministic Invariants" script proves — with exact BigInt math against on-chain state at a pinned block — merkle root integrity, per-leaf proof validity, and cumulative preservation (no entitlement shrinks or disappears while unclaimed) for every selected artifact. Voters/delegators delta-exclusivity is proven only when both mainnet targets were selected and the script output shows that cross-target check ran. Lines marked "restatement credits" mean a repo-reviewed, root-pinned restatement ledger (e.g. the Union reassignment) was credited out of the deltas for exactly its own epoch — moved prior-round principal, not a current-week payment; do not flag those amounts. A baseline resolved from a ".superseded.json" artifact and lines marked "justified removal (restatement, claimed==0)" mean a committed restatement is verified but not yet accepted on-chain: the active root's tree is preserved next to the restated one, and each cleared removal is proven unclaimed by an exact-deficit match. Do not flag that pending state.
