...
const fetchVotemarketClaimedBounties = async (block_min: number, block_max: number) => {
    ...
    const promises = Object.keys(MAINNET_VM_PLATFORMS).map(async (protocol) => {
        ...
        const paddedLocker = pad(MAINNET_VM_PLATFORMS[protocol].locker as `0x${string}`, { size: 32 }).toLowerCase();


        const response = await getLogsByAddressAndTopics(MAINNET_VM_PLATFORMS[protocol].platform, block_min, block_max, { "0": claimedEventHash, "1": paddedLocker });
...
const fetchWardenClaimedBounties = async (block_min: number, block_max: number) => {
    ...
    const logPromises = distributorAddresses.map((distributor, index) => {
        return new Promise(async (resolve) => {
            // manage rate limits
            setTimeout(async () => {
                const logsResponse = await getLogsByAddressAndTopics(distributor, block_min, block_max, { "0": claimedEventHash, "3": paddedBotmarket });
 ...
 */
const fetchHiddenHandClaimedBounties = async (publicClient: PublicClient, period: number, block_min: number, block_max: number) => {
    ...
    const claimedResponse = await getLogsByAddressAndTopics(getAddress("0xa9b08B4CeEC1EF29EdEC7F9C94583270337D6416"), block_min, block_max, { "0": rewardClaimedHash });
    ...
    for (let i = 0; i < numChunks; i++) {
        ...
        const depositedBribeResponse = await getLogsByAddressAndTopics(getAddress("0xE00fe722e5bE7ad45b1A16066E431E47Df476CeC"), block_min, block_max, { "0": depositBribeHash });
        ...
        for (const log of depositedBribeResponse.result) {
            ...
            // Filter out old ones
            if (Number(decodedLog.args.deadline) < period || getAddress(decodedLog.args.market) != HH_BALANCER_MARKET) {
                continue;
            }
