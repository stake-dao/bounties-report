...
export const generateMerkle = async (
  ...
  if (lastMerkle) {
    ...
    switch (network) {
      case ETHEREUM:
        usersClaimedAddress =
          await getAllAccountClaimedSinceLastFreezeWithAgnostic(
            tokenToDistribute,
...
async function getClosestBlockTimestamp(
  chain: string,
  timestamp: number
): Promise<number> {
  try {
    ...
    console.error("Error fetching closest block timestamp:", error);
    throw error;
  }
}
